...
void handleSysexCallback(byte command, byte argc, byte* argv)
{
  if (!FirmataExtInstance->handleSysex(command, argc, argv)) {
    Firmata.sendString(F("Unhandled sysex command")); //what is 0xF1=241?
	//for(int i=0;i<argc;i++){Firmata.sendStringf(F("%d"),8,argv[i]);} //added this to see what was rejected
  }
}
