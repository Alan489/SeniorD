  ...
  if (command != FREQUENCY_COMMAND)
  {
	  ...
	  if (frequencyCommand == FREQUENCY_SUBCOMMAND_CLEAR)
	  {
		  if (_activePin == pin || (_activePin >= 0 && pin == 0x7F))
		  {
			  ...
			  _activePin = -1;
			  Firmata.sendString(F("Detached interrupt for frequency measurement"));
		  }
	  }
  	  // Set or query
	  if (frequencyCommand == FREQUENCY_SUBCOMMAND_QUERY)
	  {
		  ...
		  if (_activePin == -1)
		  {
			  ...
			  switch (mode)
			  {
				  case INTERRUPT_MODE_DISABLE:
				  Firmata.sendString(F("DISABLE"));
				  internalMode = -1;
				  break;
				  case INTERRUPT_MODE_LOW:
				  Firmata.sendString(F("LOW"));
				  internalMode = LOW;
				  break;
				  case INTERRUPT_MODE_HIGH:
				  Firmata.sendString(F("HIGH"));
				  internalMode = HIGH;
				  break;
				  case INTERRUPT_MODE_FALLING:
				  Firmata.sendString(F("FALLING"));
				  internalMode = FALLING;
				  break;
				  case INTERRUPT_MODE_RISING:
				  Firmata.sendString(F("RISING interrupt"));
				  internalMode = RISING;
				  break;
				  case INTERRUPT_MODE_CHANGE:
				  Firmata.sendString(F("CHANGE"));
				  internalMode = CHANGE;
				  break;
				  default:
				  Firmata.sendString(F("NO VALID MODE"));
				  internalMode = -1;
				  break;
			  }
			  if (internalMode >= 0)
			  ...
			  Firmata.sendStringf(F("delay %ld on pin %ld"), 8, (int32_t)_reportDelay, (int32_t)pin);
		  }
